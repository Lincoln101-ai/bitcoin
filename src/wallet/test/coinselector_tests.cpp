    ...
    for (int i = 0; i < RUN_TESTS; i++)
    {
      ...
      // test randomness
      {
          ...
          for (int i = 0; i < RUN_TESTS; i++) {
            ...
            BOOST_CHECK(testWallet.SelectCoinsMinConf(50.5 * COIN, filter_standard, vCoins, setCoinsRet2, nValueRet, coin_selection_params));
            ...
            for (int j = 0; j < RANDOM_REPEATS; j++)
            {
                ...
                BOOST_CHECK(testWallet.SelectCoinsMinConf(0.5 * COIN, filter_standard, vCoins, setCoinsRet , nValueRet, coin_selection_params));
                BOOST_CHECK(testWallet.SelectCoinsMinConf(0.5 * COIN, filter_standard, vCoins, setCoinsRet2, nValueRet, coin_selection_params));
          ...
          for (int i = 0; i < RUN_TESTS; i++) {
            ...
            for (int j = 0; j < RANDOM_REPEATS; j++)
            {
                ...
                // run the test RANDOM_REPEATS times and only complain if all of them fail
                BOOST_CHECK(testWallet.SelectCoinsMinConf(90*CENT, filter_standard, vCoins, setCoinsRet , nValueRet, coin_selection_params));
                BOOST_CHECK(testWallet.SelectCoinsMinConf(90*CENT, filter_standard, vCoins, setCoinsRet2, nValueRet, coin_selection_params));
                if (equal_sets(setCoinsRet, setCoinsRet2))
                    fails++;
            }
            BOOST_CHECK_NE(fails, RANDOM_REPEATS);
