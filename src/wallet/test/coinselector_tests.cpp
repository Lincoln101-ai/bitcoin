...
BOOST_AUTO_TEST_CASE(knapsack_solver_test)
{
    ...
    for (int i = 0; i < RUN_TESTS; i++)
    {
        ...
        BOOST_CHECK(KnapsackSolver(34 * CENT, GroupCoins(vCoins), setCoinsRet, nValueRet));
        ...
        BOOST_CHECK(KnapsackSolver( 9 * CENT, GroupCoins(vCoins), setCoinsRet, nValueRet));
        ...
        BOOST_CHECK(KnapsackSolver(71 * CENT, GroupCoins(vCoins), setCoinsRet, nValueRet));
        BOOST_CHECK(!KnapsackSolver(72 * CENT, GroupCoins(vCoins), setCoinsRet, nValueRet));

        // now try making 16 cents.  the best smaller coins can do is 6+7+8 = 21; not as good at the next biggest coin, 20
        BOOST_CHECK(KnapsackSolver(16 * CENT, GroupCoins(vCoins), setCoinsRet, nValueRet));
        ...
        BOOST_CHECK(KnapsackSolver(11 * CENT, GroupCoins(vCoins), setCoinsRet, nValueRet));
        ...
        BOOST_CHECK(KnapsackSolver(95 * CENT, GroupCoins(vCoins), setCoinsRet, nValueRet));
        BOOST_CHECK_EQUAL(nValueRet, 1 * COIN);  // we should get 1 BTC in 1 coin
        BOOST_CHECK_EQUAL(setCoinsRet.size(), 1U);

        BOOST_CHECK(KnapsackSolver(195 * CENT, GroupCoins(vCoins), setCoinsRet, nValueRet));
        ...
        BOOST_CHECK(KnapsackSolver(MIN_CHANGE, GroupCoins(vCoins), setCoinsRet, nValueRet));
        ...
        BOOST_CHECK(KnapsackSolver(1 * MIN_CHANGE, GroupCoins(vCoins), setCoinsRet, nValueRet));
        ...
        BOOST_CHECK(KnapsackSolver(500000 * COIN, GroupCoins(vCoins), setCoinsRet, nValueRet));
        ...
        BOOST_CHECK(KnapsackSolver(MIN_CHANGE * 10001 / 100, GroupCoins(vCoins), setCoinsRet, nValueRet));
        BOOST_CHECK_EQUAL(nValueRet, MIN_CHANGE * 10105 / 100); // we should get all coins
        BOOST_CHECK_EQUAL(setCoinsRet.size(), 3U);

        // but if we try to make 99.9, we should take the bigger of the two small coins to avoid small change
        BOOST_CHECK(KnapsackSolver(MIN_CHANGE * 9990 / 100, GroupCoins(vCoins), setCoinsRet, nValueRet));
      ...
      for (CAmount amt=1500; amt < COIN; amt*=10) {
           ...
           for (int i = 0; i < RUN_TESTS; i++) {
             BOOST_CHECK(KnapsackSolver(2000, GroupCoins(vCoins), setCoinsRet, nValueRet));
      ...
      // test randomness
      {
          ...
          for (int i = 0; i < RUN_TESTS; i++) {
            ...
            BOOST_CHECK(KnapsackSolver(50.5 * COIN, GroupCoins(vCoins), setCoinsRet , nValueRet));
            BOOST_CHECK(KnapsackSolver(50.5 * COIN, GroupCoins(vCoins), setCoinsRet2, nValueRet));
            ...
            for (int j = 0; j < RANDOM_REPEATS; j++)
            {
                ...
                BOOST_CHECK(KnapsackSolver(0.5 * COIN, GroupCoins(vCoins), setCoinsRet , nValueRet));
                BOOST_CHECK(KnapsackSolver(0.5 * COIN, GroupCoins(vCoins), setCoinsRet2, nValueRet));
...
BOOST_AUTO_TEST_CASE(ApproximateBestSubset)
{
    ...
    BOOST_CHECK(KnapsackSolver(1003 * COIN, GroupCoins(vCoins), setCoinsRet, nValueRet));
