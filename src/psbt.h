...
struct PartiallySignedTransaction
{
    ...
    inline void Serialize(Stream& s) const {
        ...
        if (m_version != std::nullopt && *m_version >= 2) {
            ...
            SerializeToVector(s, static_cast<uint32_t>(inputs.size()));
            SerializeToVector(s, CompactSizeWriter(PSBT_GLOBAL_OUTPUT_COUNT));
            SerializeToVector(s, static_cast<uint32_t>(outputs.size()));
        }
        ...
        while(!s.empty()) {
            ...
            switch(type) {
                ...
                case PSBT_GLOBAL_INPUT_COUNT:
                {
                    ...
                    UnserializeFromVector(s, input_count);
                ...
                case PSBT_GLOBAL_OUTPUT_COUNT:
                {
                    ...
                    UnserializeFromVector(s, output_count);
